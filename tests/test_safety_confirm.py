from orion.safety.confirm import requires_confirmation
import pytest
...
def _reset_confirmation_state():
    from orion.safety.confirm import reset_turn_state
...
async def test_ask_confirmation_stops_active_spinner_before_prompt():
    from orion.safety.confirm import ask_confirmation

    with patch("orion.ui.spinner.stop_active_spinner", new=AsyncMock()) as mock_stop, \
         patch("orion.safety.confirm._SESSION.prompt_async", new=AsyncMock(return_value="yes")):
...
async def test_ask_confirmation_accepts_yes_or_y_in_any_case(answer):
    ...
    with patch("orion.ui.spinner.stop_active_spinner", new=AsyncMock()), \
         patch("orion.safety.confirm._SESSION.prompt_async", new=AsyncMock(return_value=answer)):
        result = await ask_confirmation("Delete file?")
...
async def test_ask_confirmation_cancels_on_non_yes_text():
    ...
    with patch("orion.ui.spinner.stop_active_spinner", new=AsyncMock()), \
         patch("orion.safety.confirm._SESSION.prompt_async", new=AsyncMock(return_value="anything else")):
        result = await ask_confirmation("Delete file?")
...
async def test_ask_command_confirmation_formats_exact_command():
    from orion.safety.confirm import ask_command_confirmation

    with patch("orion.safety.confirm.ask_confirmation", new=AsyncMock(return_value=True)) as mock_ask:
...
async def test_ask_file_action_confirmation_formats_delete_with_full_path():
    from orion.safety.confirm import ask_file_action_confirmation
...
async def test_repeated_same_file_action_denial_is_short_circuited():
    ...
    with patch("orion.ui.spinner.stop_active_spinner", new=AsyncMock()), \
         patch("orion.safety.confirm._SESSION.prompt_async", new=AsyncMock(return_value="no")) as mock_prompt:
...
async def test_denial_of_one_action_does_not_block_different_action():
    ...
    with patch("orion.ui.spinner.stop_active_spinner", new=AsyncMock()), \
         patch("orion.safety.confirm._SESSION.prompt_async", new=AsyncMock(side_effect=["no", "yes"])) as mock_prompt:
        first = await ask_file_action_confirmation("delete", source_path="/home/jadon/a.txt")
